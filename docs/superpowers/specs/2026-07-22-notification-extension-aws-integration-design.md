...
- Date: 2026-07-22
- Status: Approved for implementation in separate sessions
...
- the Misskey fork changes required to consume the internal URL and shared
  secret from environment variables
- the extension image release and health-check requirements
- the internal migration path for `miwkey-notification-importer`
- deployment, rollback, logging, and verification

This design does not cover:

- the Misskey 2025 application image cutover itself
...
`feature/asg-stabilization` contains only:

- the existing changes through `e9f190e`
...
## 6. miwkey-extension Repository Handoff

Implementation in `miwkey-extension` is intentionally assigned to a separate
session.

### 6.1 Misskey Configuration

Update the embedded Misskey fork so notification extension configuration can
come from these environment variables:
...
secret are present. Preserve the existing optional timeout behavior.

Add unit tests for environment-only configuration, YAML fallback, environment
override, and the incomplete-pair disabled case.
...
- publish both `linux/amd64` and `linux/arm64` in one manifest
- publish the immutable `sha-<commit>` tag
- optionally publish `latest`, but never consume it from ECS
...
the branch must exist and the workflow must be verified after the branch setup.

Use Buildx with the platform configuration needed for both architectures. Add
QEMU setup if the selected GitHub runner requires emulation for the ARM build.
...
The ECS task definition must use this container health check so Service Connect
does not route initial traffic to an unhealthy extension task.

### 6.4 Logging Contract

Write application logs to stdout/stderr and never log notification payloads,
JWTs, Authorization headers, or the shared secret.

The AWS deployment keeps two log groups with six-month retention:
...
3. Complete the `miwkey-extension` repository handoff requirements.
...
### miwkey-extension handoff

- Go tests pass.
- Misskey configuration precedence tests pass.
- The container health check fails when `/healthz` is unavailable and succeeds
  when the service is ready.
- The GHCR manifest contains both amd64 and arm64 images.
- The integration test using DynamoDB Local still uses auto-create successfully.
